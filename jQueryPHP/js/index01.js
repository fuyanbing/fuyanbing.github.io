	...
	$.ajax({
		...
		success : function (response, status, xhr) {
			...
			$.each($('.editor'), function (index, value) {
				...
				var sumOne = arr[index].substr(199,1);//获取最后一位
				var sumTwo = arr[index].substr(198,2);//获取最后两位
				
				//小于等于200位
				if (sumOne == '<') {
					summary[index] = arr[index].substr(0,199);
				}
				if (sumTwo == '</') {
...
//												$('.comment_list').eq(index).find('.load_more').html('加载更多评论1');
												$('.comment_list').eq(index).find('.load_more').html('<span class="ui-button-text">加载更多评论</span>');
												page++;
												if (page > count) {
													$('.comment_list').eq(index).find('.load_more').off('click');
													$('.comment_list').eq(index).find('.load_more').hide();
...
//						content:$('.uEditorIframe').contents().find('#iframeBody').html(),
    ...
    var editor = new wangEditor('uEditorCustom');
    editor.config.menus = [
	    'bold',
	    'underline',
	    'italic',
	...
	// 初始化编辑器的内容
    editor.$txt.html('请输入问题描述');
    
    $('.wangEditor-txt').click(function () {
        // 清空内容。
        // 不能传入空字符串，而必须传入如下参数
        editor.$txt.html('');
    });
	...
	$('#logout').click(function() {
		$.removeCookie('user');
		window.location.href = '/jQuery/jQueryProject/index.html';
	});
	//数据正在加载中
	$('#loading').dialog({
		autoOpen: false,
		...
		width: 180,
		height: 50
	}).siblings('.ui-widget-header').hide();
	//点击注册弹框
	...
	$('#reg').dialog({
		...
		highlight: function(element, errorClass) {
			$(element).css('border', '1px solid #CC1F27');
