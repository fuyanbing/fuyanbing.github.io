...
class db_mysql {
	...
	function insert($table, $field_values) {
		$field_names = $this->getCol ( 'DESC ' . $table );       //使用DESC+表名查询表字段信息
		$fields = array ();                                      //定义字段数组
		$values = array ();                                      //定义值数组
		foreach ( $field_names as $value ) {                     //循环字段结果集数组
		    //如果指定的字段已经存在于$field_values关联数组的键或索引中
			if (array_key_exists ( $value, $field_values ) == true) {
				$fields [] = $value;                             //将该值赋给字段数组元素
				$values [] = "'" . $field_values [$value] . "'"; //将值添加到值列表
			}
		}
		if (! empty ( $fields )) {                                //如果字段列表不为空
		    //根据字段数组和值数组中的元素构建INSERT语句，implode用来将数组元素变成以逗号分隔的字符串
	...
	function delete($table,$where=''){
		if(empty($where)){                                //如果没有传入了$where参数
